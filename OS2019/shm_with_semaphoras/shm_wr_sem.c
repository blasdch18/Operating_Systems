...
void shared_memory_(int, struct shmseg*, int);
...
int main(int argc, char *argv[]) {
   ...
   if (pid > 0) {
      shared_memory_(pid, shmp, initializer);
   } else if (pid == 0) {
      shared_memory_(pid, shmp, initializer);
      return 0;
   } else {
...
void shared_memory_(int pid, struct shmseg *shmp, int initializer) {
   int cntr;
   int numtimes;
   int sleep_time;
   int semid;
   ...
   for (numtimes = 0; numtimes < 1000; numtimes++) {
      ...
      if(shmp->buffer %2 ==0){
         shmp->par = shmp->buffer;
      }else{
         shmp->impar = shmp->buffer;
      }
